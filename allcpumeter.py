...
def gpu_info():
    ...
    if rc == 0:
        for line in out.splitlines():
            low=line.lower()
            if not any(k in low for k in ('vga compatible controller','3d controller','display controller')):
                continue
            vendor='unknown'
            if 'intel' in low: vendor='intel'
            elif 'nvidia' in low: vendor='nvidia'
            elif 'amd' in low or 'ati' in low: vendor='amd'
            m=re.search(r'(?:VGA compatible controller|3D controller|Display controller):\s*(.*)$', line, re.I)
...
def scan():
    gpus=gpu_info()
    # The GUI performs the more expensive Intel counter probe after discovery.
    return {
        'cpu':cpu_info(), 'network':{'primary':default_interface()},
        'disk':{'root_device':root_device()}, 'gpus':gpus,
        'sensors':sensors_status(),
        'intel_gpu':{'installed':shutil.which('intel_gpu_top') is not None,'usable':None,'error':'','engines':[]},
...
def intel_gpu_stats(force=False):
    ...
    cache=cache_dir/'intel-gpu.json'; lock_path=cache_dir/'intel-gpu.lock'
    ...
    try:
        with lock_path.open('w') as lock:
            ...
            if not shutil.which('intel_gpu_top'):
                ...
                rc,out,err=run(['intel_gpu_top','-J','-s','500','-n','2','-o','-'],timeout=3.5)
                if rc != 0:
...
def generate_conky(hw,s):
    ...
    if s['show_gpu'] and gpus:
        g=gpus[0]; vendor=g['vendor']; lines += ['',section('GPU METER')]
        for name_line in display_name_lines(g['description'],s.get('gpu_name_mode','crop'),s.get('gpu_custom_name',''),width):
            lines.append('${color AAAAAA}'+name_line)
        base=''
        if vendor=='intel' and hw['intel_gpu']['installed']: base=f'python3 {py} --helper-gpu intel'
        elif vendor=='nvidia' and hw['nvidia']['installed']: base=f'python3 {py} --helper-gpu nvidia'
        elif vendor=='amd' and hw['amd_busy_path']: base=f'python3 {py} --helper-gpu amd --amd-path {shlex.quote(hw["amd_busy_path"])}'
        if base:
            if vendor=='intel':
                metrics=[]
                ...
                lines += ['${color C77DFF}GPU ${alignr}${execi 2 '+base+'}%','${color C77DFF}${execibar 2 7,0 '+base+'}']
                if s['trendlines']: lines.append('${color C77DFF}${execigraph 2 "'+base+'" 28,0 C77DFF C77DFF 100}')
        else: lines.append('${color AAAAAA}Utilization backend unavailable')
...
def launch_gui():
    ...
    class App(tk.Tk):
        def __init__(self):
            ...
            self.hw={}; self.settings=load_settings(); self.vars={}; self.build(); self.refresh()
        ...
        def build_display(self):
            ...
            for name,label,default in [('show_swap','Show swap',True),('show_gpu','Show GPU',True),('show_network','Show network',True),('show_disk','Show root disk',True)]: ttk.Checkbutton(f,text=label,variable=self.v(name,default)).pack(anchor='w')
        ...
        def refresh(self):
            self.config(cursor='watch'); self.update_idletasks(); self.hw=scan()
            if any(g.get('vendor')=='intel' for g in self.hw.get('gpus',[])) and self.hw['intel_gpu']['installed']:
                self.hw['intel_gpu']=intel_gpu_probe()
            self.config(cursor='')
            h=self.hw; lines=[f"CPU: {h['cpu']['model']}",f"Logical CPUs: {h['cpu']['logical_cpus']}",'',f"Primary network interface: {h['network']['primary']}",f"Root disk device: {h['disk']['root_device'] or 'not determined'}",'']
            if h['gpus']:
                for i,g in enumerate(h['gpus'],1): lines.append(f"GPU {i}: {g['vendor'].upper()} - {g['description']}")
            else: lines.append('GPU: none detected by lspci')
            lines += ['', 'lm-sensors: '+('available' if h['sensors']['usable'] else 'installed but no readable temperatures' if h['sensors']['installed'] else 'not installed')]
            if h['sensors']['usable']: lines.append(f"Per-core temperature labels found: {h['sensors']['core_temps']}")
            if any(g.get('vendor')=='intel' for g in h.get('gpus',[])) and h['intel_gpu']['installed']:
                if h['intel_gpu'].get('usable'):
                    lines.append('Intel GPU counters: available')
                    if h['intel_gpu'].get('engines'): lines.append('Intel GPU engines: '+', '.join(h['intel_gpu']['engines']))
                else:
                    lines.append('Intel GPU counters: unavailable')
                    if h['intel_gpu'].get('error'): lines.append('intel_gpu_top: '+h['intel_gpu']['error'])
            self.hw_text.delete('1.0','end'); self.hw_text.insert('1.0','\n'.join(lines)); self.render_deps()
        def render_deps(self):
            for w in self.dep.winfo_children(): w.destroy()
            row=0
            def dep_line(text,package=None):
                nonlocal row; ttk.Label(self.dep,text=text).grid(row=row,column=0,sticky='w');
                if package: ttk.Button(self.dep,text='Install',command=lambda p=package:self.install_pkg(p)).grid(row=row,column=1,padx=8)
                row+=1
            dep_line('CPU temperatures (lm-sensors): '+('Installed' if self.hw['sensors']['installed'] else 'Not installed'),None if self.hw['sensors']['installed'] else 'lm-sensors')
            ...
            if 'intel' in vendors:
                if not self.hw['intel_gpu']['installed']:
                    ...
                    dep_line('Intel GPU utilization: installed, but counters are not readable')
                else:
                    dep_line('Intel GPU utilization (intel-gpu-tools): Available')
            if 'nvidia' in vendors: dep_line('NVIDIA GPU utility (nvidia-smi): '+('Available' if self.hw['nvidia']['installed'] else 'Not available'))
            if 'amd' in vendors: dep_line('AMD kernel utilization counter: '+('Available' if self.hw['amd_busy_path'] else 'Not exposed'))
...
def main():
    ...
    if args.helper_temp:
        ...
        helper_gpu(args.helper_gpu,args.amd_path,args.metric)
    else:
        launch_gui()
